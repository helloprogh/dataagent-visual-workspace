...
OpenCode2 的 `prompt` 是异步入队接口；界面侧的 `/agent` 是持续到当前运行完成的 AG-UI SSE 请求。因此界面既能看到异步任务状态，又能获得一个标准、完整的 AG-UI run。
...
| `POST /agent` | `POST /api/session/:sessionID/prompt` | 提交最新 user message，立即返回 inbox item |
...
| `session.tool.called/progress` | 工具执行中；task 类工具补充 `subagent.started/progress` |
| `session.tool.success/failed` | `TOOL_CALL_END / TOOL_CALL_RESULT` + 子 Agent 终态 |

## 4. 思考过程

`GET /api/event` 中的 `session.reasoning.started/delta/ended` 转换为标准 `REASONING_*` 事件。模型不提供 reasoning 时，界面不会伪造真实思考内容；`dev:scenario` 只额外提供一段明确标记的调试 reasoning。

## 5. 同步 / 异步任务状态

| 界面接口 | OpenCode2 接口 | 用途 |
| --- | --- | --- |
| `POST /agent` | `POST /api/session/:sessionID/prompt` | 异步入队，状态通过同一 AG-UI SSE 返回 |
...
界面状态包括 `queued`、`delivered`、`running`、`retry`、`waiting_permission`、`completed` 和错误/中断。
...
| `GET /debug/capabilities` | 连接、版本、9 个场景、8 组接口与事件映射 |
| `POST /agui/hybrid` | 真实 OpenCode2 + 可视工作区调试事件 |
