...
test('conversation header omits share and export buttons while keeping other actions', async () => {
  const source = await frontend('features/conversation/components/ConversationHeader.vue')
  ...
  assert.match(header, /emit\('toggleAudit'\)/)
  assert.match(header, /emit\('toggleDeliverables'\)/)
  assert.match(header, /chat\.(?:running|ready)/)
  const chat = await frontend('features/conversation/components/AgentChat.vue')
  assert.match(chat, /<ConversationHeader/)
  assert.match(chat, /event\.shiftKey && event\.key\.toLocaleLowerCase\(\) === 'e'/)
})
...
test('conversation presentation is Element-Plus-X over direct AG-UI client', async () => {
  ...
  const composer = await frontend('features/conversation/components/ConversationComposer.vue')
  assert.match(agentChat, /<ConversationComposer/)
  assert.match(composer, /<XSender/)
  assert.match(agentChat, /<Welcome/)
  assert.match(composer, /<XSender[\s\S]*<template #prefix>[\s\S]*chat\.addFile[\s\S]*<ModelSelector[\s\S]*<\/XSender>/)
  assert.match(agentChat, /presentationItems/)
  assert.match(agentChat, /useConversationPresentation/)
  assert.match(await frontend('features/conversation/composables/useConversationPresentation.ts'), /buildPresentation\(messages.value, running.value, activeReasoningId.value\)/)
  ...
  assert.match(composer, /\.composer-input-actions :deep\(\.model-selector\) \{ margin-left: 0; \}/)
  assert.match(composer, /:deep\(\.chat-write-input\) \{ color: var\(--da-text-primary\); caret-color: var\(--da-text-emphasis\); \}/)
...
test('conversation files open a pushed preview panel and approvals stay backend-schema driven', async () => {
  ...
  assert.match(chat, /<ConversationInspector/)
  const inspector = await frontend('features/conversation/components/ConversationInspector.vue')
  assert.match(inspector, /<FilePreviewPanel[\s\S]*v-if="activePreview"/)
  assert.match(inspector, /<DeliverablesPanel[\s\S]*v-else-if="deliverablesOpen"/)
  assert.match(inspector, /<AuditPanel[\s\S]*v-else-if="auditOpen"/)
  ...
  assert.match(chat, /useConversationArtifacts/)
  assert.match(artifacts, /generatedFilesFromTool/)
  assert.match(artifacts, /agui\/workspace-archive/)
  assert.match(chat, /:deliverable-count="deliverables.length"/)
  assert.match(await frontend('features/conversation/components/ConversationHeader.vue'), /deliverableCount \}\}<\/small>/)
