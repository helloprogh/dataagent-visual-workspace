...
test('conversation presentation is Element-Plus-X over direct AG-UI client', async () => {
  ...
  assert.match(processGroup, /\.process-group__content \{[^}]*gap: 0;/)
  assert.match(processGroup, /:deep\(\.tool-call-list\) \{ margin-top: 0; \}/)
  assert.match(processGroup, /:deep\(\.tool-call summary\)[^}]*min-height: 1\.5rem; padding-block: 0; padding-left: 0;/)
  assert.match(processGroup, /:deep\(\.reasoning-card::before\) \{ display: none; \}/)
