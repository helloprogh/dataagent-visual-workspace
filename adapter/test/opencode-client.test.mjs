...
test('answers and rejects OpenCode questions in the configured instance directory', async () => {
  const calls = []
  const client = new OpenCodeClient({
    baseUrl: 'http://127.0.0.1:4096',
    workspaceDirectory: 'D:\\ProjectSpace\\dataagent',
  ...
  await client.replyQuestion('que_123', [['生产'], ['表', '自定义范围']])
  await client.rejectQuestion('que_456')

  const reply = new URL(calls[0].url)
  const reject = new URL(calls[1].url)
  assert.equal(reply.pathname, '/api/question/que_123/reply')
  assert.equal(reject.pathname, '/api/question/que_456/reject')
  assert.equal(reply.searchParams.get('directory'), 'D:\\ProjectSpace\\dataagent')
  assert.equal(reject.searchParams.get('directory'), 'D:\\ProjectSpace\\dataagent')
