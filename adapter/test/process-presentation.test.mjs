...
test('waiting feedback covers initial send and text placeholders without replaying history', async () => {
  ...
  const presentation = await fs.readFile(new URL('../../frontend/src/features/conversation/composables/useConversationPresentation.ts', import.meta.url), 'utf8')
  assert.match(chat, /useConversationPresentation/)
  assert.match(presentation, /responsePhase.value === 'responding'\) return Boolean\(activeTextId.value\) &&/)
