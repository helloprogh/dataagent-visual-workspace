...
const workspaceWidgetSchema = z.object({
  ...
  component: z.string().describe('Registered component name, for example ui.kpis or ui.lineChart'),
...
useFrontendTool({
  ...
  handler: async ({ title, subtitle, widgets }) => {
...
useFrontendTool({
  ...
  handler: async ({ orchestrator, agents, timeline, activities }) => {
