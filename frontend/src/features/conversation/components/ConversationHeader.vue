...
<template>
    <header class="agent-chat__header" :class="{ 'agent-chat__header--compact': compact }">
      ...
      <div class="agent-chat__header-actions">
        <button type="button" :class="{ active: auditOpen }" :aria-pressed="auditOpen" :aria-label="t('chat.record')" :title="t('chat.record')" @click="emit('toggleAudit')"><svg viewBox="0 0 20 20" aria-hidden="true"><circle cx="10" cy="10" r="6.5"/><path d="M10 6v4l2.5 1.5"/></svg><span>{{ t('chat.record') }}</span></button>
        <button type="button" :class="{ active: deliverablesActive }" :aria-pressed="Boolean(deliverablesActive)" :aria-label="`${t('chat.deliverables')} ${deliverableCount}`" :title="t('chat.deliverables')" @click="emit('toggleDeliverables')">
        ...
        <span role="status" :class="{ active: running, pending: pendingApprovalCount, failed: error && !running }"><i></i>{{ hydrating ? t('chat.restoring') : running ? t('chat.running') : pendingApprovalCount ? t('interrupt.needsAction') : error ? t('chat.incomplete') : t('chat.ready') }}</span>
      </div>
    </header>
</template>
...
.agent-chat__header--compact .agent-chat__identity small { display: none; }
