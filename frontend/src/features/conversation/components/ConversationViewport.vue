    ...
    >
      ...
      <div v-else-if="!messages.length && !running" class="agent-welcome">
        <div class="agent-welcome__brand">
          ...
          <span class="agent-welcome__eyebrow">DATA AGENT</span>
          <div class="agent-welcome__title"><h1>DATA AGENT</h1></div>
      ...
      <div v-else class="message-list">
        <div v-if="nextCursor" class="load-older">
          <el-button text :loading="loadingOlder" @click="emit('loadOlder')">Earlier</el-button>
        </div>
        ...
        <div v-if="showResponsePending" class="response-pending" role="status" aria-live="polite">
          ...
          <span>{{ responsePhase === 'responding' ? 'Organizing response…' : 'Waiting for response…' }}</span>
        </div>
      </div>
    </div>
