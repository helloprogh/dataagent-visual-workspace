...
The browser has exactly one backend dependency: the AG-UI SSE endpoint configured by `VITE_AGUI_URL` (the local default is `/api/agui`, proxied to `/agent`). It does not call session, permission, context, health, or capability endpoints.
...
Pending interrupts are stored with the local conversation snapshot so a browser refresh can restore the approval card without calling a session or permission lookup endpoint. The Adapter persists the corresponding server-side correlation inside its existing thread/session registry.
