...
- `node scripts/live-permission-smoke.mjs` 新增外部拒绝后再次 hydration 检查。真实会话 `ses_f7eed827effewTAAsdYc8RbrjR` 已通过：权限要求 ask，首次恢复1项；直接调用上游 reject 后再次恢复 success，本地待办清空。不执行受保护操作。
- hydration 不再只信任非空缓存；按表单/权限接口的成功结果核对。接口故障保留该类缓存；并发待办/回执变更优先保留较新的本地状态。
- 首次探针失败的错误曾被清理阶段404覆盖，脚本现先记录主错误，并仅对仍存在的探针进行拒绝清理；该失败不计为通过。
- 浏览器过期待办清除使用 mock 回归；真实证据为权限服务与适配器接口联调，不能称为真实跨客户端浏览器全链路验收。重新进入/刷新生效，未提供后台实时同步，未解决此前工具拒绝后的上游持久化终态问题。

### 真实浏览器多权限与外部拒绝

- 使用同样的真实服务和 Playwright 环境执行 `node scripts/live-permission-reconciliation.mjs`。只创建无真实文件操作的权限探针；从不提交 always，不生成模型内容。
- 真实会话 `ses_f7e41fef8ffee2DDSgfKXLrlCh` 验收通过：刷新发现新权限（1→2），外部 reject 后恢复为0，输入框恢复、缓存清空；零新增 Run、零页面异常。补齐上一节 mock 浏览器与真实接口分开的证据边界。
- 原先期望拒绝一项后保留另一项的测试失败；实际上此版本上游会拒绝同会话全部权限，已依据真实结果与上游实现修正断言。批量拒绝产生重复请求的适配器风险另行验证。
- 创建探针前只读等待 explore 外部目录规则为 ask，实际探针返回非 ask 仍立即失败，不覆盖权限规则或放宽安全策略。
